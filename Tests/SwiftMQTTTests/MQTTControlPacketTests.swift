...
struct MQTTControlPacketTests {
    ...
    @Test("Decode Connack Packet") func decodeConnackPacket() {
        ...
        #expect(
            connackPacket.fixedHeader == FixedHeader(type: .CONNACK, flags: 0, remainingLength: 2))
        #expect(
    ...
    @Test("Create Connect Packet") func createConnectPacket() {
        ...
        #expect(
            connectPacket.fixedHeader == FixedHeader(type: .CONNECT, flags: 0, remainingLength: 19))
        #expect(
            connectPacket.varHeader.encode() == [
                0x00, 0x04, 0x4d, 0x51, 0x54, 0x54, 0x04, 0x02, 0x00, 0x3c,
            ])
        #expect(
            connectPacket.payload.encode() == [
                0x00, 0x07, 0x73, 0x77, 0x69, 0x66, 0x74, 0x2D, 0x31,
            ])
    }
    ...
    @Test("Decode Publish Packet") func deocdePublishPacket() {
        ...
        #expect(
            publishPacket.fixedHeader == FixedHeader(type: .PUBLISH, flags: 0, remainingLength: 17))
    ...
    @Test("Create Subscribe Packet") func createSubscribePacket() {
        ...
        #expect(
            subscribePacket.fixedHeader
                == FixedHeader(type: .SUBSCRIBE, flags: 2, remainingLength: 15))
    ...
    @Test("Create disconnect") func createDisconnect() {
        ...
        #expect(
            disconnect.fixedHeader == FixedHeader(type: .DISCONNECT, flags: 0, remainingLength: 0))
    }
