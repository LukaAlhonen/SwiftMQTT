...
public struct ConnackProperties: Properties {
    ...
    public init(
        ...
        if let assignedClientIdentifier {
            self.assignedClientIdentifier = Property.assignedClientIdentifier(
        ...
        if let wildcardSubscriptionAvailable {
            self.wildcardSubscriptionAvailable = Property.wildcardSubscriptionAvailable(
                wildcardSubscriptionAvailable)
        }
        if let subscriptionIdentifierAvailable {
            self.subscriptionIdentifierAvailable = Property.subscriptionIdentifierAvailable(
                subscriptionIdentifierAvailable)
        }
        if let sharedSubscriptionAvailable {
            self.sharedSubscriptionAvailable = Property.sharedSubscriptionAvailable(
                sharedSubscriptionAvailable)
        }
        if let serverKeepalive { self.serverKeepalive = Property.serverKeepalive(serverKeepalive) }
        if let responseInformation {
            self.responseInformation = Property.responseInformation(responseInformation)
        }
...
public struct ConnackVariableHeader: Equatable, Sendable {
    ...
    public init(
        sessionPresent: UInt8, connectReturnCode: ConnectReturnCode? = nil,
        connectReasonCode: ConnectReasonCode? = nil, connackProperties: ConnackProperties? = nil
    ) {
...
extension Connack {
    public init(bytes: Bytes, version: Version) throws {
        ...
        case .v3:
            if remaining.count != 2 {
                throw MQTTError.protocolViolation(
                    .malformedPacket(reason: .decodeError("malformed variable header")))
            }
    ...
    {
        self.varHeader = ConnackVariableHeader(
            sessionPresent: sessionPresent ? 1 : 0, connectReasonCode: reasonCode,
            connackProperties: properties)
        self.fixedHeader = FixedHeader(
            type: .CONNACK, flags: 0, remainingLength: UInt(self.varHeader.encode().count))
    }
}
