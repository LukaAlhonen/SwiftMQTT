...
extension MQTTClientnew {
    ...
    private func tryConnect() async throws {
        try await self.connection.send(packet: Connect(clientId: self.clientId, keepAlive: 60))
...
extension MQTTClientnew {
    func subscribe(to topics: [TopicFilter]) async throws {
            ...
            try await self.connection.send(packet: subscribePacket)
